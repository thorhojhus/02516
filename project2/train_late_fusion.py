...
from datasets import FrameVideoDataset, VideoFileDataset
...
for epoch in range(num_epochs):
    ...
    print(f'Epoch [{epoch+1}/{num_epochs}], Loss: {epoch_loss:.4f}, Acc: {train_correct/len(train_framevideo_dataset):.4f}')
    model.eval()
    val_loss = 0.0
    val_correct = 0
    with torch.no_grad():
